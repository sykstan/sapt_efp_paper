...
% remade figures for Reviewer 2
\begin{figure}
    \centering
    \includegraphics[scale = 0.8]{images/c3mim-br-all-conf.pdf}
    \caption{Different configurations of \protect\ipair{mim}{3}{br} \label{fig:conf-c3mim-br}}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[scale = 0.7]{images/c2mpyr-ntf2-all-conf.pdf}
    \caption{Different configurations of \protect\ipair{mpyr}{2}{ntf} \label{fig:conf-c2mpyr-ntf2}}
\end{figure}
...
\corrected{
This is in line with the results presented by Parker \emph{et al.}, where for SAPT2+3, the basis set with the lowest mean absolute error was aug-cc-pVDZ. (Tables III and IV, pp. 094106-7,8)
\cite{Parker2014a}
This holds true across all the different subsets, which are categorisations of the systems based on the dominating interactions, such as hydrogen-bonding, dispersion, and mixed.
}

To further validate this choice, a number of test calculations were performed using aug-cc-pVTZ, indicating that aug-cc-pVDZ gave satisfactory accuracy.
...
\corrected{
The consistency between the two basis sets (E(aug-cc-pVTZ) - E(aug-cc-pVDZ)) is immediately apparent: with the triple-$\zeta$ basis set, electrostatics is always weaker compared to the double-$\zeta$ basis set, by 2.7 \enUnit~ on average (3.0 \enUnit~ for imidazolium systems and 1.9 \enUnit~for pyrrolidinium systems). 
Exchange follows the same trend, with the larger basis set giving a larger exchange energy of 4.9 \enUnit~on average.
...
In the total interaction energy, the largest difference of 9 \enUnit~ comes from \ipair{mim}{1}{cl}, specifically for the in-plane configuration p2.
